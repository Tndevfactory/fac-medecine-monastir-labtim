...
const ensureUniqueIndex = async (
  ...
  try {
    ...
    if (results.length === 0) {
      console.log(
        `Creating unique index '${indexName}' on ${tableName}.${columnName}...`
      );
      ...
      console.log(
        `Unique index '${indexName}' on ${tableName}.${columnName} created successfully.`
      );
    } else {
      console.log(
        `Unique index '${indexName}' on ${tableName}.${columnName} already exists.`
      );
    }
  } catch (error) {
    console.error(
      `Error ensuring unique index '${indexName}' on ${tableName}.${columnName}:`,
      error
    );
...
const initializeDB = async () => {
  try {
    ...
    console.log(
      "All models synchronized with the database. Associations wired up."
    );
    ...
    try {
      ...
      await ensureUniqueIndex(
        "carousel_items",
        "order",
        "carousel_items_order_unique",
        transaction
      ); // Ensure carousel order is unique
      await ensureUniqueIndex(
        "presentation_content",
        "sectionName",
        "presentation_content_sectionName_unique",
        transaction
      ); // Ensure sectionName is unique
      await ensureUniqueIndex(
        "publications",
        "doi",
        "publications_doi_unique",
        transaction
      ); // Ensure DOI is unique
      ...
      console.error(
        "Error during custom unique index creation transaction:",
        indexError
      );
...
app.use((err, req, res, next) => {
  ...
  if (err.name === "MulterError") {
    ...
    statusCode = 400;
    message = `Duplicate entry for a unique field: ${
    ...
    statusCode = 500;
    message =
      "Database synchronization error: A foreign key constraint already exists. This might be a transient issue during startup. Try restarting.";
    console.warn(
      "Caught ER_FK_DUP_NAME in global error handler. This often indicates a transient sync issue, not a runtime app error."
    );
  } else if (
    err.name === "SequelizeDatabaseError" &&
    err.parent &&
    err.parent.code === "ER_FK_CANNOT_OPEN_PARENT"
  ) {
    statusCode = 500;
    message =
      "Database synchronization error: Failed to open referenced table (e.g., Users table not found/ready). Ensure model sync order and database readiness.";
    console.error(
      "Caught ER_FK_CANNOT_OPEN_PARENT in global error handler. This means a referenced table was not found during FK creation."
    );
  }
