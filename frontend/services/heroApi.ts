...
export const getHeroSection = async (): Promise<{
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        errorData.message || "Failed to fetch hero section data."
      );
    }
 ...
 */
export const saveHeroSection = async (
  formData: FormData,
  token: string
): Promise<{ success: boolean; data: Hero; message?: string }> => {
  try {
    const response = await fetch(`${API_BASE_URL}/hero`, {
    ...
    throw new Error(
      `Failed to save hero section: ${error.message || "Unknown error"}`
    );
  }
};
