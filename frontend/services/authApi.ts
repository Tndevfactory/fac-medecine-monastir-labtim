// services/authApi.ts
import { User, AuthResponse } from '@/types/index'; // Assuming AuthResponse type includes token and user object
 ...
 * @returns A promise that resolves to the AuthResponse.
 */
export const loginUser = async (credentials: Pick<User, 'email' | 'password'>): Promise<AuthResponse> => {
  try {
 ...
 * @returns A promise that resolves to the AuthResponse.
 */
export const initialAdminSignup = async (userData: Pick<User, 'name' | 'email' | 'password'>): Promise<AuthResponse> => {
  try {
