      ...
      const serializableBlocks = currentContentBlocks.map((block, index) => {
        if (block.type === 'image') {
          ...
          if (imageBlockToSerialize.file) {
            formData.append(`image_${imageBlockToSerialize.id}`, imageBlockToSerialize.file); // Use block ID for unique file key
          }
          return {
            id: imageBlockToSerialize.id,
            type: imageBlockToSerialize.type,
            // If there's a new file, URL will be set by backend. If URL is explicitly cleared (null/empty/blob), send null.
