...
const MasterPFEList = () => {
  ...
  const [activeFilterDropdown, setActiveFilterDropdown] = useState<
    string | null
  >("year"); // 'year' open by default
  const [selectedYear, setSelectedYear] = useState<number | null>(null);
  const [selectedType, setSelectedType] = useState<string | null>(null); // State for type filter
  const [expandedMasterPFEsId, setExpandedMasterPFEsId] = useState<
    string | null
  >(null); // State for expanded summary
  ...
  const fetchMasterPFEs = useCallback(async () => {
    ...
    try {
      const response = await getAllMasterSIs(
        token,
        undefined,
        selectedYear,
        selectedType,
      ...
      if (response.success && response.data) {
        ...
        setErrorModal({
          ...
          briefDescription:
            response.message ||
            "Une erreur est survenue lors de la récupération des Masters et PFEs.",
  ...
  const typeOptions = useMemo(
    () => Array.from(new Set(masterPFEs.map((item) => item.type))),
    [masterPFEs]
  ); // Get unique types from fetched data
  ...
  const currentMasterPFEs = masterPFEs.slice(
    startIndex,
  ...
  const toggleFilterDropdown = (dropdownName: string) => {
    setActiveFilterDropdown(
      activeFilterDropdown === dropdownName ? null : dropdownName
    );
  };
  ...
  return (
    <section className={`py-16 bg-white ${inter.variable}`}>
      {toast && (
        <Toast
          ...
          <div className="w-full lg:w-1/4 bg-white p-6 rounded-lg border border-gray-200 shadow-sm lg:sticky lg:top-24 lg:self-start">
              ...
              >
                ...
                <ChevronDown
                  className={`h-5 w-5 transition-transform duration-200 ${
                    activeFilterDropdown === "type" ? "rotate-180" : "rotate-0"
                  }`}
                />
              </button>
            ...
            <div className="mb-6">
              ...
              >
                ...
                <ChevronDown
                  className={`h-5 w-5 transition-transform duration-200 ${
                    activeFilterDropdown === "year" ? "rotate-180" : "rotate-0"
                  }`}
                />
              </button>
            ...
            {loading ? (
              <div className="flex justify-center items-center py-12">
                ...
                <span className="ml-3 text-lg text-gray-700">
                  Chargement des Masters et PFEs...
                </span>
              </div>
              ...
              <div className="text-center text-gray-600 text-lg py-12">
                {error
                  ? `Erreur: ${error}`
                  : "Aucun Master ou Stage Ingénieur ne correspond à la sélection."}
              </div>
            )}
