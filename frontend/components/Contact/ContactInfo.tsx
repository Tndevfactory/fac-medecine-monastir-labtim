...
const ContactInfo = () => {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setTimeout(() => {
      // Replaced alert with a console log for better user experience in Canvas
      console.log('Message sent successfully!');
      setFormData({ nom: '', prenom: '', email: '', sujet: '', contenu: '' }); // Clear form
      setIsSending(false); // Reset animation state
    }, 1000); // Animation duration + a bit of delay
  };

  return (
    <section className={`py-16 bg-white ${inter.className}`}>
      {/* Changed max-w-full back to max-w-7xl to constrain overall content width, adding implied padding */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Title updated to "Contact" */}
        ...
        <div className="bg-gradient-to-r from-blue-500 to-blue-600 p-8 rounded-lg shadow-xl mb-12 max-w-3xl mx-auto">
          {/* Reverted h3 to its original state without the icon */}
